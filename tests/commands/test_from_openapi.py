...
def test_from_openapi_accepts_absolute_spec_inside_root(tmp_path: Path) -> None:
    root = _make_project(tmp_path)
    spec = _FIXTURES / "petstore.json"
    dest = root / "petstore.json"
    shutil.copy(spec, dest)
    rc = run_from_openapi(
        FromOpenApiOptions(spec=str(dest), out_dir="gen"),
        project_root=root,
    )
    assert rc == 0


def test_from_openapi_rejects_absolute_spec_outside_root(tmp_path: Path) -> None:
    root = _make_project(tmp_path)
    spec = _FIXTURES / "petstore.json"
    rc = run_from_openapi(
...
def test_from_openapi_resolves_absolute_out_dir_with_dotdot(tmp_path: Path) -> None:
    ...
    rc = run_from_openapi(
        FromOpenApiOptions(spec=str(dest), out_dir=out_dir),
        project_root=root,
    )
    assert rc == 0
    assert (root / "gen" / "features").is_dir()
